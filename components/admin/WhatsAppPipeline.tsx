...
export default function WhatsAppPipeline({ reservation: res, sentBy = 'admin' }: Props) {
  ...
  const handleProposal = async (force = false) => {
    ...
    setModal({
      ...
      buttonLabel: '1. Proposition de réservation',
      buttonNum: 1,
    })
  }
  ...
  const handlePaymentRequest = async (force = false) => {
    ...
    const result = await prepareWhatsAppPaymentRequest(res.id, revolutIncluded)
    setLoading(null)
    if (!result.success) { toast.error(result.error); return }
    setModal({
      message: result.message,
      ...
      phone: result.phone,
      recipientName: result.recipientName,
      buttonLabel: '2. Demande de paiement',
      buttonNum: 2,
      showRevolutToggle: true,
      revolutIncluded,
    })
  }
  ...
  // Quand le toggle Revolut change, recharger le message
  const handleRevolutToggle = async (included: boolean) => {
    setRevolutIncluded(included)
    if (!modal || modal.buttonNum !== 2) return
    setLoading('payment_request')
    const result = await prepareWhatsAppPaymentRequest(res.id, included)
    setLoading(null)
    ...
    setModal((prev) => prev ? {
      ...
      message: result.message,
      url: result.url,
      revolutIncluded: included,
    } : null)
  }

  const steps = [
    { num: 1, label: 'Proposition', icon: MessageCircle, done: step1Done, active: true, sentAt: res.whatsapp_proposal_sent_at },
    { num: 2, label: 'Demande paiement', icon: CreditCard, done: step2Done, active: step1Done, sentAt: res.whatsapp_payment_request_sent_at },
    { num: 3, label: 'Confirmer paiement', icon: CheckCircle2, done: step3Done, active: step2Done, sentAt: res.payment_date },
    { num: 4, label: 'Envoyer fiche', icon: FileText, done: step4Done, active: step3Done, sentAt: res.whatsapp_confirmation_sent_at },
  ]
  ...
  return (
    <>
        ...
        <div className="space-y-2 pt-2 border-t border-beige-100">
            ...
            >
              ...
              {step1Done ? (
                <span className="flex items-center gap-1">
                  <Clock size={12} /> Envoyé le {res.whatsapp_proposal_sent_at ? formatDate(res.whatsapp_proposal_sent_at, 'dd/MM à HH:mm') : ''}
                </span>
          ...
          {confirmResend === 2 ? (
            ...
            >
              ...
              {step2Done ? (
                <span className="flex items-center gap-1">
                  <Clock size={12} /> Envoyé le {res.whatsapp_payment_request_sent_at ? formatDate(res.whatsapp_payment_request_sent_at, 'dd/MM à HH:mm') : ''}
                </span>
          ...
          {step3Done ? (
            <div className="p-3 bg-amber-50 border border-amber-200 rounded-xl">
              ...
              {(res as any).payment_method && (
                <p className="text-xs text-amber-600 mt-1">Moyen : {
                  (res as any).payment_method === 'orange_money' ? 'Orange Money'
                  : (res as any).payment_method === 'revolut' ? 'Revolut'
                  : (res as any).payment_method === 'virement' ? 'Virement bancaire'
                  : (res as any).payment_method
                }</p>
              )}
            </div>
            ...
            <>
              {showPaymentForm ? (
                <div className="p-3 bg-amber-50 border border-amber-200 rounded-xl space-y-2">
                  ...
                  <div>
                    <label className="text-xs text-amber-700 font-medium block mb-1">Moyen de paiement utilisé</label>
                    <select
                      value={paymentMethod}
      ...
      {/* Modale de prévisualisation WhatsApp */}
      {modal && (
