    ...
    <div className="p-6 sm:p-8 mt-14 lg:mt-0">
      ...
      {accommodations.length === 0 && (
        <div className="text-center py-20 bg-white rounded-2xl border border-beige-200">
          ...
          <Link href="/admin/hebergements/nouveau" className="btn-primary">Créer le premier hébergement</Link>
        </div>
      )}
    </div>
  )
...
function AccommodationAdminCard({ accommodation: acc }: { accommodation: any }) {
  const statusColor = acc.status === 'active' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
  return (
    <div className="bg-white rounded-2xl border border-beige-200 overflow-hidden hover:shadow-card transition-shadow">
      ...
      <div className="p-5">
        <p className="text-xs text-dark/50 mb-1">{acc.partner?.name}</p>
        ...
        <div className="grid grid-cols-3 gap-2 text-xs text-dark/60 mb-4">
          <span className="flex items-center gap-1"><Users size={11} className="text-gold-500" />{acc.capacity} pers.</span>
          <span className="flex items-center gap-1"><Percent size={11} className="text-gold-500" />{acc.commission_rate}%</span>
          <span className="font-medium text-dark">{formatPrice(acc.price_per_night)}/n</span>
        </div>
