'use client'
// app/fiche/[marie_uid]/page.tsx
// Fiche invitation personnalisée — publique, design luxe fond #1A1A1A
...
export default function FicheInvitePage() {
  const params = useParams()
  const searchParams = useSearchParams()
  const marie_uid = params.marie_uid as string
  const prenom = searchParams.get('prenom') || 'Cher(e) invité(e)'
  const codeParam = searchParams.get('code') || ''

  const [data, setData] = useState<MarieData | null>(null)
  ...
  const code = codeParam || data?.code_promo || ''
  ...
  const handleDownloadPdf = async () => {
    ...
    try {
      ...
      doc.setFontSize(16)
      doc.setFont('helvetica', 'bold')
      const noms = data?.noms_maries || 'Nos Mariés'
      const nomsLines = doc.splitTextToSize(noms, 120) as string[]
      ...
      doc.setFont('helvetica', 'normal')
      let yPos = 43 + nomH
      if (data?.date_mariage) {
        doc.text(formatDate(data.date_mariage), W / 2, yPos, { align: 'center' })
        yPos += 7
      }
      ...
      doc.setFontSize(7.5)
      doc.text(data?.lieu || 'Kribi, Cameroun', W / 2, yPos, { align: 'center' })
      yPos += 8
    ...
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ background: '#1A1A1A' }}>
        <div className="text-center">
          ...
          <p className="text-sm" style={{ color: '#C9A84C' }}>Chargement de votre invitation…</p>
        </div>
      </div>
    )
  }
