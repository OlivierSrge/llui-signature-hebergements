...
export default function EmployeScanClient() {
      ...
      scannerRef.current = setInterval(() => {
        ...
        if (code?.data) {
          ...
          try {
            ...
            if (parsed.type !== 'employe' || !parsed.employe_id || !parsed.partenaire_id) {
              setError("QR invalide — ce n'est pas un QR employe L&Lui.")
              setIsScanning(false)
              return
            }
            sessionStorage.setItem('employe_id', parsed.employe_id)
            sessionStorage.setItem('employe_partenaire_id', parsed.partenaire_id)
            router.push('/employe/pin')
          } catch {
  ...
  return (
    <div className="min-h-screen bg-dark text-white flex flex-col items-center justify-center px-6 gap-8">
      ...
      <div className="text-center">
        ...
        <p className="text-white/50 text-sm mt-1">Espace Employe</p>
      </div>

      {!isScanning ? (
        <>
          <div className="w-24 h-24 rounded-full bg-gold-400/10 border-2 border-gold-400/30 flex items-center justify-center">
            <QrCode size={48} className="text-gold-400" />
          </div>

          <div className="text-center">
            <p className="text-white font-semibold text-lg">Scannez votre QR employe</p>
            <p className="text-white/40 text-sm mt-1">
              Le QR vous a ete fourni par votre responsable
            </p>
          </div>
        ...
        <div className="w-full max-w-sm flex flex-col gap-4">
          <p className="text-center text-white/60 text-sm">Pointez votre QR employe</p>
